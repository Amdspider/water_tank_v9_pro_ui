/*
 * =====================================================================
 *  SMART WATER LEVEL INDICATOR  v9.2  — Safety UI Edition
 * =====================================================================
 *
 * CHANGES FROM v7.0:
 ...
 *          ≥ highThr (94%)  → Green (full)
 *          ≥ lowThr  (20%)  → Blue (normal)
 *          < lowThr  (20%)  → Yellow (low)
 *          < 10%            → Red (critical/dry)
...
const char *SCREEN_TITLES[] = {"MAIN SCREEN",   "ALERTS",     "USAGE DATA",
                               "SYSTEM STATUS", "SMART INFO", "SETTINGS"};
#define SCR_MAIN 0
#define SCR_ALERTS 1
#define SCR_USAGE 2
#define SCR_SYSTEM 3
...
const BzNote SND_START[] = {{880, 80}, {0, 40},     {1047, 80},
                            {0, 40},   {1319, 120}, {0, 0}};
const BzNote SND_STOP[] = {{1319, 80}, {0, 30},    {1047, 80},
                           {0, 30},    {880, 120}, {0, 0}};
const BzNote SND_INFO[] = {{1047, 40}, {0, 20}, {1319, 60}, {0, 0}};
const BzNote SND_MAINT[] = {{600, 500}, {0, 0}};
const BzNote SND_RESET_DAILY[] = {{880, 80},  {0, 30}, {1047, 80},  {0, 30},
                                  {1319, 80}, {0, 30}, {1568, 100}, {0, 0}};
const BzNote SND_ERROR[] = {{2500, 180}, {0, 70},     {2100, 180},
                            {0, 70},     {1600, 260}, {0, 0}};
const BzNote SND_SLEEP[] = {{1319, 80}, {0, 40}, {1047, 80}, {0, 40},
                            {880, 80},  {0, 40}, {660, 120}, {0, 0}};
const BzNote SND_BLOCK[] = {{400, 150}, {0, 80}, {300, 200}, {0, 0}};
const BzNote SND_AUTO_DONE[] = {{1568, 70}, {0, 30}, {1319, 70}, {0, 30},
                                {1047, 90}, {0, 25}, {784, 130}, {0, 0}};
const BzNote SND_MANUAL_FULL[] = {{2500, 180}, {0, 80},  {2500, 180}, {0, 80},
                                  {1900, 260}, {0, 160}, {0, 0}};
const BzNote SND_EMERGENCY[] = {{2800, 120}, {0, 40}, {2200, 120}, {0, 40},
                                {2800, 120}, {0, 40}, {1200, 360}, {0, 0}};
...
//  CONSTANTS
// ============================================================
#define TANK_H_CM 93.0f
#define TANK_VOL_LITRES 700.0f
#define TANK_WIDTH_CM 318.0f
#define SENSOR_OFF_CM 5.0f
#define US_SAMPLES 5
#define US_TIMEOUT_US 30000UL
#define PUMP_TIMEOUT_MS 300000UL
#define PUMP_CHK_MS 30000UL
#define PUMP_MIN_RISE 2.0f
#define DRY_RUN_GRACE_MS 120000UL
#define MAX_PUMP_MS 1800000UL
#define MIN_RUN_MS 30000UL
#define MIN_REST_MS 60000UL
#define HYSTERESIS 3.0f
#define AUTO_START_MAX_LEVEL 25.0f
#define USAGE_MIN_DROP_PCT 0.20f
#define ALERT_POPUP_MS 11000UL
#define SLEEP_NOTICE_MS 12000UL
#define MAINT_HOLD_MS 3000
#define SAFE_V_LOW 180.0f
#define SAFE_V_HIGH 250.0f
#define AUTO_RETURN_MS 15000UL
#define TANK_FORCE_ON_MAX AUTO_START_MAX_LEVEL
...
#define FW_VERSION "v9.2-safety-ui"
...
#define T_ADMIN_DBG "water_tank/admin/debug"
...
const char *sysStateStr[] = {"IDLE",      "NORMAL", "REFILLING",
                             "POSS.LEAK", "LEAK!",  "ERROR"};
...
struct MovAvgV {
  float buf[MA_VOLT_WINDOW];
  int head = 0, count = 0;
  float sum = 0;
  void push(float v) {
    if (count == MA_VOLT_WINDOW)
      sum -= buf[head];
    buf[head] = v;
    sum += v;
    head = (head + 1) % MA_VOLT_WINDOW;
    if (count < MA_VOLT_WINDOW)
      count++;
  }
  float avg() { return count ? sum / count : 0; }
...
volatile bool lowLevelAlarm = false, lowLevelMuted = false;
...
volatile unsigned long alertPopupUntil = 0;
...
volatile unsigned long hbSensor = 0, hbControl = 0, hbButton = 0, hbDisplay = 0,
                       hbMqtt = 0;
volatile float levelAtStart = 0;
volatile float lastUsageLevel = -1.0f;
...
volatile bool alertPopupActive = false;
volatile AlertLevel alertPopupLevel = AL_INFO;
char alertPopupMsg[32] = "";
char alertPopupDetail[40] = "";
...
//  UTILITY MACROS
// ============================================================
#define TAKE_MUTEX(m, ms)                                                      \
  (xSemaphoreTakeRecursive(m, pdMS_TO_TICKS(ms)) == pdTRUE)
...
bool normalizeSettings() {
  bool changed = false;
  if (fabsf(cfg.tankH - TANK_H_CM) > 0.1f) {
    cfg.tankH = TANK_H_CM;
    changed = true;
  }
  if (fabsf(cfg.tankVolLitres - TANK_VOL_LITRES) > 0.1f) {
    cfg.tankVolLitres = TANK_VOL_LITRES;
    changed = true;
  }
  if (cfg.lowThr < 10.0f || cfg.lowThr > AUTO_START_MAX_LEVEL) {
    cfg.lowThr = 20.0f;
    changed = true;
  }
  if (cfg.highThr < 93.0f || cfg.highThr > 97.0f ||
      cfg.highThr < cfg.lowThr + 10.0f) {
    cfg.highThr = 94.0f;
    changed = true;
  }
  if (cfg.highThr < cfg.lowThr + 10.0f) {
    cfg.highThr = cfg.lowThr + 10.0f;
    changed = true;
  }
  cfg.lowThr = constrain(cfg.lowThr, 10.0f, AUTO_START_MAX_LEVEL);
  cfg.highThr = constrain(cfg.highThr, 90.0f, 97.0f);
  return changed;
}

void loadSettings() {
  if (EEPROM.read(EE_MAGIC_A) != EE_MAGIC) {
    saveSettings();
    return;
  }
  Settings t;
  EEPROM.get(EE_SETTINGS, t);
  if (t.crc == crc8((const uint8_t *)&t, sizeof(Settings) - 1)) {
    cfg = t;
...
void saveHealth() {
  PumpHealth oldH;
  EEPROM.get(EE_HEALTH, oldH);
  health.crc = crc8((const uint8_t *)&health, sizeof(PumpHealth) - 1);
  if (oldH.runSecs != health.runSecs || oldH.cycles != health.cycles) {
    EEPROM.put(EE_HEALTH, health);
    EEPROM.commit();
  }
}
void loadHealth() {
  PumpHealth h;
  EEPROM.get(EE_HEALTH, h);
  if (h.crc == crc8((const uint8_t *)&h, sizeof(PumpHealth) - 1))
    health = h;
}
void saveLitres() {
  float oldL;
  EEPROM.get(EE_LITRES, oldL);
  if (isnan(oldL) || fabsf(oldL - totalLitres) > 0.5f) {
    EEPROM.put(EE_LITRES, totalLitres);
    EEPROM.commit();
  }
}
void loadLitres() {
  EEPROM.get(EE_LITRES, totalLitres);
  if (isnan(totalLitres) || totalLitres < 0)
    totalLitres = 0;
}
void saveUsage() {
  float oldU[24];
  EEPROM.get(EE_USAGE, oldU);
  bool changed = false;
  for (int i = 0; i < 24; i++)
    if (isnan(oldU[i]) || fabsf(oldU[i] - hourlyUsage[i]) > 0.1f)
      changed = true;
  if (changed) {
    EEPROM.put(EE_USAGE, (float *)hourlyUsage);
    EEPROM.commit();
  }
}
void loadUsage() {
  float buf[24];
  EEPROM.get(EE_USAGE, buf);
  for (int i = 0; i < 24; i++)
    hourlyUsage[i] = (isnan(buf[i]) || buf[i] < 0) ? 0.0f : buf[i];
}
void loadSecrets() {
  prefs.begin(NVS_NS, false);
  if (prefs.isKey("hmac_key"))
    prefs.getString("hmac_key", g_hmacSecret, sizeof(g_hmacSecret));
  if (prefs.isKey("ota_pass"))
    prefs.getString("ota_pass", g_otaPass, sizeof(g_otaPass));
  if (prefs.isKey("broker_ca"))
    prefs.getString("broker_ca", g_brokerCA, sizeof(g_brokerCA));
  prefs.end();
}
...
// ============================================================
void pushAlert(AlertLevel lv, AlertType ty, const char *msg,
               const char *detail = "");
...
void IRAM_ATTR echoISR() {
  if (digitalRead(ECHO_PIN))
    ...
    unsigned long d = micros() - echoStart;
    if (d > 0 && d < US_TIMEOUT_US) {
      echoDur = d;
      echoReady = true;
    }
  }
  ...
  if (TAKE_MUTEX(xStateMutex, 20)) {
    ...
    if (ty != AT_NONE) {
      for (int n = 0; n < ALERT_HISTORY; n++) {
        if (alertHistory[n].active && alertHistory[n].type == ty) {
          i = n;
          break;
        }
      }
    ...
    alertHistory[i].level = lv;
    alertHistory[i].type = ty;
    alertHistory[i].active = true;
    strncpy(alertHistory[i].msg, msg, 31);
    alertHistory[i].msg[31] = '\0';
    strncpy(alertHistory[i].detail, detail ? detail : "", 39);
    alertHistory[i].detail[39] = '\0';
    alertPopupLevel = lv;
    strncpy(alertPopupMsg, msg, 31);
    alertPopupMsg[31] = '\0';
    strncpy(alertPopupDetail, detail ? detail : "", 39);
    alertPopupDetail[39] = '\0';
    alertPopupActive = true;
    alertPopupUntil = millis() + ALERT_POPUP_MS;
    struct tm ti;
    if (getLocalTime(&ti)) {
      alertHistory[i].hour = ti.tm_hour;
      alertHistory[i].minute = ti.tm_min;
    } else {
      alertHistory[i].hour = 0;
      alertHistory[i].minute = 0;
    }
    recountActiveAlerts();
    uiRedraw = true;
...
static const BzNote *patternForId(int id) {
  ...
  case PAT_MAINT:
    return SND_MAINT;
  case PAT_RESET_DAILY:
    return SND_RESET_DAILY;
  case PAT_ERROR:
    return SND_ERROR;
  case PAT_SLEEP:
    return SND_SLEEP;
  case PAT_BLOCK:
    return SND_BLOCK;
  case PAT_AUTO_DONE:
    return SND_AUTO_DONE;
  case PAT_MANUAL_FULL:
    return SND_MANUAL_FULL;
  case PAT_EMERGENCY:
    return SND_EMERGENCY;
  default:
    return nullptr;
  }
}
  ...
  for (;;) {
    ...
    if (xQueueReceive(xBuzzerQueue, &req, 0) == pdTRUE) {
      ...
      if (req.pat == PAT_IDLE) {
        ...
        if (ns) {
          ...
          noteIdx = 0;
          noteEnd = 0;
          duty = (req.pat == PAT_MANUAL_FULL || req.pat == PAT_EMERGENCY) ? 255
                                                                          : 180;
        }
      }
    }
    if (seq && millis() >= noteEnd) {
      ...
      if (n.ms == 0 && n.freq == 0) {
        ...
        if (n.freq > 0) {
          ledcWriteTone(BZ_CH, n.freq);
          ledcWrite(BZ_CH, duty);
        } else {
          ledcWrite(BZ_CH, 0);
          ledcWriteTone(BZ_CH, 0);
        }
...
bool canStartMotor(char *reason) {
  if (sysState == SYS_ERROR || leakConfirmed) {
    strncpy(reason, "Error active", 63);
    return false;
  }
  if (voltWarn && !voltWarnMuted) {
    snprintf(reason, 63, "Unsafe voltage! %.0fV", smoothVoltage);
    return false;
  }
  if (!isVoltSafe(smoothVoltage) && opMode != MODE_MANUAL) {
    snprintf(reason, 63, "Voltage out of range %.0fV", smoothVoltage);
    return false;
  }
  if (opMode == MODE_AUTO && !maLevel.full()) {
    strncpy(reason, "Waiting for stable level", 63);
    return false;
  }
  if (opMode == MODE_AUTO && smoothLevel > AUTO_START_MAX_LEVEL) {
    snprintf(reason, 63, "Auto blocked >%.0f%%", AUTO_START_MAX_LEVEL);
    return false;
  }
  return true;
}
...
void emergencyStop(const char *reason) {
  ...
  strncpy(pumpStopReason, reason ? reason : "emergency stop",
          sizeof(pumpStopReason) - 1);
  ...
  if (mqtt.connected())
    mqtt.publish(T_ALERT, reason);
}

void requestPumpStop(const char *reason) {
  if (stopPending || !pumpRunning)
    return;
  strncpy(pumpStopReason, reason ? reason : "requested",
          sizeof(pumpStopReason) - 1);
  pumpStopReason[sizeof(pumpStopReason) - 1] = '\0';
  stopPending = true;
  stopPendTime = millis();
...
void startPump(const char *reason) {
  if (pumpRunning || pumpStartPend || stopPending)
    return;
  strncpy(pumpStartReason, reason ? reason : "requested",
          sizeof(pumpStartReason) - 1);
...
void updatePumpShutdown() {
  if (pumpStartPend) {
    ...
    if (!canStartMotor(reason)) {
      PUMP("Motor ON cancelled - requested=%s block=%s", pumpStartReason,
           reason);
      pumpStartPend = false;
      pendStart = 0;
      return;
    ...
    pumpOnTime = millis();
    pumpTimedOut = false;
    levelAtStart = smoothLevel;
    pumpToutChk = millis();
    dryRunTime = millis() + DRY_RUN_GRACE_MS;
    dryRunChk = false;
    health.cycles++;
    usageUpd = millis();
    updateFillEta();
    uiRedraw = true;
    PUMP("Motor ON - reason=%s level=%.1f%% volt=%.0fV", pumpStartReason,
         smoothLevel, smoothVoltage);
    if (mqtt.connected())
      mqtt.publish(T_PUMP, "ON", true);
  }
  if (stopPending && millis() - stopPendTime >= 3000UL) {
    ...
    digitalWrite(RELAY_PIN, LOW);
    health.runSecs += (millis() - pumpOnTime) / 1000;
    pumpRunning = stopPending = false;
    pumpStopTime = millis();
    stopBuzzer();
    uiRedraw = true;
    PUMP("Motor OFF - reason=%s ran %.0fs", pumpStopReason,
         (millis() - pumpOnTime) / 1000.0f);
    if (mqtt.connected())
...
void updateFillEta() {
  if (!pumpRunning || smoothLevel >= cfg.highThr) {
    fillEtaMin = 0;
    return;
  }
...
void updateUsage() {
  ...
  float drop = lastUsageLevel - smoothLevel;
  if (drop >= USAGE_MIN_DROP_PCT) {
    float litres = (drop / 100.0f) * cfg.tankVolLitres;
    totalLitres += litres;
    dailyUsage += litres;
    struct tm ti;
    if (getLocalTime(&ti))
      hourlyUsage[ti.tm_hour] += litres;
    lastUsageLevel = smoothLevel;
  } else if (drop < 0.0f) {
    lastUsageLevel = smoothLevel;
  }
...
void doNightSleep() {
  ...
  if (pumpRunning) {
    ...
    while (pumpRunning && millis() - t < 5000)
  ...
  if (TAKE_MUTEX(xTftMutex, 500)) {
    ...
    tft.setCursor(38, 70);
    tft.print(F("Wake: 06:00 AM"));
    tft.setCursor(44, 82);
    tft.print(F("Good night"));
    struct tm ti;
    if (getLocalTime(&ti)) {
      char tb[12];
      snprintf(tb, 12, "Now: %02d:%02d", ti.tm_hour, ti.tm_min);
  ...
  vTaskDelay(pdMS_TO_TICKS(SLEEP_NOTICE_MS));
...
void SensorTask(void *pv) {
  for (;;) {
    ...
    if (usState == US_WAIT) {
      ...
      bool rdy = echoReady;
      unsigned long dur = echoDur;
      if (rdy)
        echoReady = false;
      portENABLE_INTERRUPTS();
      if (rdy) {
        float d = (dur * 0.0343f) / 2.0f;
        if (d >= 0 && d <= cfg.tankH + SENSOR_OFF_CM + 10)
          usBuf[usN++] = d;
        usIdx++;
        usState = (usIdx >= US_SAMPLES)     ? US_CALC
                  : (millis() - usTmr > 50) ? US_TRIG
                                            : US_WAIT;
      } else if (millis() - usTmr > 40) {
        usIdx++;
        usState = (usIdx >= US_SAMPLES) ? US_CALC : US_TRIG;
      }
    }
    if (usState == US_CALC) {
      if (usN > 0) {
        ...
        if (TAKE_MUTEX(xStateMutex, 10)) {
          waterPrev = waterLevel;
          waterLevel = lvl;
          maLevel.push(waterLevel);
          smoothLevel = maLevel.avg();
          GIVE_MUTEX(xStateMutex);
        }
    ...
    // Voltage FSM
    if (voltFSM == VS_IDLE) {
      if (millis() - vLastChk >= VOLT_CHK_MS) {
        vLastChk = millis();
        vDcSum = 0;
        vDcN = 0;
        vAcSumSq = 0;
      ...
      if (micros() - vTimer >= (unsigned long)VOLT_SAMP_US) {
        vTimer = micros();
        if (voltFSM == VS_DC) {
          vDcSum += (analogRead(VOLT_PIN) / 4095.0f) * 3.3f;
          vDcN++;
          if (vDcN >= VOLT_DC_SAMP) {
            vDcBias = vDcSum / vDcN;
            voltFSM = VS_AC;
          }
        } else if (voltFSM == VS_AC) {
          float v = ((analogRead(VOLT_PIN) / 4095.0f) * 3.3f - vDcBias);
          vAcSumSq += v * v;
          vAcN++;
          if (vAcN >= VOLT_AC_SAMP)
            voltFSM = VS_DONE;
          ...
          if (TAKE_MUTEX(xStateMutex, 10)) {
            currentVoltage = cv;
            maVolt.push(cv);
            smoothVoltage = maVolt.avg();
            GIVE_MUTEX(xStateMutex);
          }
          if (millis() - lastVoltPrint >= 1000) {
            lastVoltPrint = millis();
            VOLT("AC=%.1fV smooth=%.1fV %s", currentVoltage, smoothVoltage,
                 isVoltSafe(smoothVoltage) ? "OK" : "FAULT");
          }
          if (maVolt.full() && millis() - bootTimeMs > 12000 &&
              !voltWarnMuted) {
            if (!isVoltSafe(smoothVoltage)) {
              if (!voltIsAbnormal) {
                voltIsAbnormal = true;
                voltAbnormalTimer = millis();
              } else if (millis() - voltAbnormalTimer >= 5000) {
                if (pumpRunning) {
                  emergencyStop("Voltage unsafe while running");
                }
                if (!voltWarn && !voltWarnMuted) {
                  voltWarn = true;
                  char det[40];
                  snprintf(det, 40, "%.0fV (safe:%.0f-%.0fV)", smoothVoltage,
                           cfg.safeVLow, cfg.safeVHigh);
                  pushAlert(AL_CRITICAL, AT_VOLTAGE, "High AC Voltage", det);
                  // [V8-5] No buzzer in auto mode for voltage alert
                  if (opMode == MODE_MANUAL)
                    playBuzzer(PAT_ERROR);
                  setLED(true, false, false);
                }
              }
            } else {
...
void pushLevelHistory(float lvl) {
  ...
  float dL = levelHistory[(levelHistIdx - 1 + SLOPE_WINDOW) % SLOPE_WINDOW] -
             levelHistory[oldest];
  float dT =
      (levelHistoryTime[(levelHistIdx - 1 + SLOPE_WINDOW) % SLOPE_WINDOW] -
       levelHistoryTime[oldest]) /
      60000.0f;
...
void updateSystemState() {
  ...
  if (sysState == SYS_ERROR) {
    if (!pumpRunning && usN > 0 && !leakConfirmed)
      sysState = SYS_IDLE;
    else
      return;
  }
  if (pumpRunning)
    sysState = SYS_REFILLING;
  else if (leakConfirmed)
    sysState = SYS_CONFIRMED_LEAK;
  else if (lastLeakScore >= 4 && lastLeakScore < SCORE_THRESHOLD)
    sysState = SYS_POSSIBLE_LEAK;
  else if (currentSlope < -0.5f)
    sysState = SYS_NORMAL_USE;
  else
    sysState = SYS_IDLE;
  if (sysState != prev)
    INFO("State: %s→%s", sysStateStr[prev], sysStateStr[sysState]);
}
...
void updateLeakScore() {
  ...
  if (millis() - leakTimer < LEAK_EVAL_MS)
    return;
  leakTimer = millis();
  float drop = leakSnap - smoothLevel;
  leakSnap = smoothLevel;
  if (drop > 0)
    leakTotalDrop += drop;
  else
    leakTotalDrop = max(0.0f, leakTotalDrop - 0.5f);
  if (fabsf(drop) <= LEAK_PAUSE_STABLE)
    pauseTimer = millis();
  bool noPause = (millis() - pauseTimer > 120000UL);
  int score = 0;
  if (currentSlope < -0.05f)
    score += 2;
  if (noPause)
    score += 2;
  if (currentSlope < LEAK_SLOPE_MAX && currentSlope > LEAK_SLOPE_MIN)
    score += 2;
  if (leakTotalDrop > LEAK_TOTAL_DROP_THR)
    score += 1;
  lastLeakScore = score;
  if (score >= SCORE_THRESHOLD && !leakConfirmed) {
    leakConfirmed = true;
    sysState = SYS_CONFIRMED_LEAK;
    pushAlert(AL_CRITICAL, AT_LEAK, "Leak Detected",
...
void checkWaterLimits() {
  if (sysState == SYS_ERROR)
    return;
  if (opMode == MODE_MANUAL && smoothLevel < cfg.lowThr && !pumpRunning &&
      !lowLevelAlarm && !lowLevelMuted && maLevel.full() &&
      millis() - bootTimeMs > 10000) {
    lowLevelAlarm = true;
    char det[40];
    snprintf(det, sizeof(det), "Below %.0f%% manual level", cfg.lowThr);
    pushAlert(AL_WARNING, AT_LOW_LEVEL, "Low Water Level", det);
    playBuzzer(PAT_ERROR, true);
    setLED(true, true, false);
    uiRedraw = true;
    WARN("Manual low level alarm: %.1f%%", smoothLevel);
  } else if (lowLevelAlarm && (opMode != MODE_MANUAL || pumpRunning ||
                               smoothLevel >= cfg.lowThr + HYSTERESIS)) {
    lowLevelAlarm = false;
    stopBuzzer();
    clearAlertType(AT_LOW_LEVEL);
  }
  if (smoothLevel >= cfg.lowThr + HYSTERESIS) {
    lowLevelMuted = false;
  }

  // Very low / dry run — only buzzer in manual
  if (smoothLevel < 10.0f && !pumpRunning && !veryLow && !veryLowMuted &&
      maLevel.full() && millis() - bootTimeMs > 10000) {
    veryLow = true;
    if (opMode == MODE_MANUAL)
      playBuzzer(PAT_ERROR, true); // [V8-5]
    setLED(true, false, false);
  ...
  if (opMode == MODE_MANUAL && pumpRunning && !stopPending &&
      smoothLevel >= cfg.highThr) {
    if (!manualFullAlarm) {
      manualFullAlarm = true;
      manualFullAlarmAt = millis();
      pushAlert(AL_CRITICAL, AT_HIGH_LEVEL, "Tank Full",
                "Manual motor still ON");
      ...
      pushAlert(AL_CRITICAL, AT_HIGH_LEVEL, "Emergency Stop",
                "Tank full, motor OFF");
  ...
  if (smoothLevel >= cfg.highThr && !tankFullAlerted && !manualFullAlarm) {
    tankFullAlerted = true;
    pushAlert(AL_WARNING, AT_HIGH_LEVEL, "High Water Level",
              "Tank above high setpoint");
    if (opMode == MODE_MANUAL && !manualFullAlarm)
      playBuzzer(PAT_INFO); // [V8-5]
    uiRedraw = true;
    if (opMode == MODE_MANUAL && pumpRunning && !manualFullAlarm)
      requestPumpStop("manual full level");
    INFO("Tank FULL: %.1f%%", smoothLevel);
  }
  if (smoothLevel < cfg.highThr) {
    tankFullAlerted = false;
    if (!manualFullAlarm)
      clearAlertType(AT_HIGH_LEVEL);
  }
}
...
void controlPump() {
  if (leakConfirmed) {
    if (pumpRunning)
      requestPumpStop("leak detected");
    return;
  }
  if (stopPending || opMode != MODE_AUTO)
    return;
  if (!inSchedule()) {
    if (pumpRunning)
      requestPumpStop("outside schedule");
    return;
  }
  if (!maLevel.full() || millis() - bootTimeMs < 15000)
    return;
  bool restOk = (millis() - pumpStopTime >= MIN_REST_MS);
  if (!pumpRunning && !pumpStartPend && !pumpTimedOut && restOk) {
    bool autoStartAllowed = (smoothLevel <= AUTO_START_MAX_LEVEL);
    if (autoStartAllowed && smoothLevel <= cfg.lowThr)
      startPump("auto low level");
    else if (autoStartAllowed && predictedLevel < cfg.lowThr)
      startPump("predicted low level");
  }
  if (pumpRunning && !stopPending) {
    bool runOk = (millis() - pumpOnTime >= MIN_RUN_MS);
    if (runOk && smoothLevel >= cfg.highThr)
      requestPumpStop("auto high level reached");
  }
}
...
void updatePumpTimeout() {
  if (!pumpRunning || millis() - pumpToutChk < PUMP_CHK_MS)
    return;
  pumpToutChk = millis();
  ...
  if (elapsed >= (PUMP_TIMEOUT_MS / 1000.0f) && rise < PUMP_MIN_RISE) {
    ...
    pushAlert(AL_CRITICAL, AT_PUMP_TIMEOUT, "Pump Timeout",
              "No level rise detected");
  }
}

void checkDryRun() {
  if (dryRunChk || !pumpRunning || millis() < dryRunTime)
    return;
  dryRunChk = true;
  if (smoothLevel - levelAtStart < 1.0f) {
    char reason[64];
    snprintf(reason, sizeof(reason), "Dry run - no rise after %lus",
             DRY_RUN_GRACE_MS / 1000UL);
    emergencyStop(reason);
    pushAlert(AL_CRITICAL, AT_DRY_RUN, "Dry Run Protected",
              "Motor auto stopped");
  }
}
...
void ControlTask(void *pv) {
  for (;;) {
    ...
    if (TAKE_MUTEX(xStateMutex, 20)) {
      ...
      if (opMode != MODE_MAINTENANCE) {
        ...
        if (pumpRunning) {
          ...
          if (millis() - pumpOnTime > MAX_PUMP_MS) {
            ...
            pushAlert(AL_CRITICAL, AT_PUMP_MAXRUN, "Max Pump Runtime",
                      "Auto stopped for safety");
          }
        }
      }
      updatePumpShutdown();
      ...
      if (millis() - litresSaveTmr > 300000) {
        litresSaveTmr = millis();
        GIVE_MUTEX(xStateMutex);
        saveLitres();
        saveHealth();
        saveUsage();
      ...
      if (millis() - lastDebugPrint >= 5000) {
        ...
        INFO("lvl=%.1f%% volt=%.1fV pump=%s mode=%s state=%s", smoothLevel,
             smoothVoltage, pumpRunning ? "ON" : "OFF",
             opMode == MODE_AUTO     ? "AUTO"
             : opMode == MODE_MANUAL ? "MANUAL"
                                     : "MAINT",
             sysStateStr[sysState]);
      }

      if (isSleepTime() && !sleepRequested && !pumpRunning && !pumpStartPend &&
          !leakConfirmed && millis() - bootTimeMs > 30000) {
        sleepRequested = true;
        GIVE_MUTEX(xStateMutex);
        doNightSleep();
        TAKE_MUTEX(xStateMutex, 20);
...
void adjustSettingsStep(int dir) {
  ...
  case 1:
    cfg.lowThr += 5.0f * dir;
    if (cfg.lowThr > AUTO_START_MAX_LEVEL)
      cfg.lowThr = 10.0f;
    if (cfg.lowThr < 10.0f)
      cfg.lowThr = AUTO_START_MAX_LEVEL;
    if (cfg.highThr < cfg.lowThr + 10.0f)
      cfg.highThr = min(97.0f, cfg.lowThr + 10.0f);
    g_pendingSave = true;
    playBuzzer(PAT_CLICK);
    break;
  case 2:
    cfg.highThr += 1.0f * dir;
    if (cfg.highThr > 97.0f)
      cfg.highThr = max(90.0f, cfg.lowThr + 10.0f);
    if (cfg.highThr < cfg.lowThr + 10.0f)
      cfg.highThr = 97.0f;
    if (cfg.highThr < cfg.lowThr + 10.0f)
      cfg.highThr = cfg.lowThr + 10.0f;
...
void applySettingsStep() { adjustSettingsStep(1); }
...
void checkDualHold() {
  ...
  if (b1 && !g_btn1Down) {
    g_btn1Down = true;
    g_btn1DownAt = millis();
    g_btn1HoldStart = millis();
  }
  if (!b1) {
    g_btn1Down = false;
    g_maintFired = false;
    g_btn1LongFired = false;
    g_btn1HoldStart = 0;
  }
  if (b2 && !g_btn2Down) {
    g_btn2Down = true;
  ...
  if (g_btn1Down && !g_btn2Down && !g_btn1LongFired &&
      currentScreen == SCR_SETTINGS) {
    if (g_btn1HoldStart == 0)
      g_btn1HoldStart = millis();
...
void handleButton(AceButton *btn, uint8_t evt, uint8_t) {
  ...
  if (evt == AceButton::kEventClicked)
  ...
  playBuzzer(PAT_CLICK);
  DBG("Button %s clicked scr=%d", pin == BTN1_PIN ? "BTN1" : "BTN2",
      currentScreen);
  ...
  if (manualFullAlarm) {
    ...
    stopBuzzer();
    if (pumpRunning)
      requestPumpStop("manual full acknowledged");
    dismissAlert();
    uiRedraw = true;
  ...
  if (veryLow || lowLevelAlarm || voltWarn || pumpTimedOut || leakConfirmed) {
    if (veryLow)
      veryLowMuted = true;
    if (lowLevelAlarm)
      lowLevelMuted = true;
    ...
    leakConfirmed = false;
    leakClearCount = 0;
    if (sysState == SYS_ERROR || sysState == SYS_CONFIRMED_LEAK)
      sysState = SYS_IDLE;
    dismissAlert();
    uiRedraw = true;
    return;
  ...
  if (currentScreen == SCR_MAIN) {
    if (opMode == MODE_AUTO) {
      ...
      if (pumpRunning)
        requestPumpStop("manual button");
      else
        startPump("manual button");
    }
    uiRedraw = true;
    return;
  }
...
void ButtonTask(void *pv) {
  for (;;) {
    ...
    checkDualHold();
    if (currentScreen != SCR_MAIN &&
        millis() - lastButtonTime > AUTO_RETURN_MS &&
        millis() - bootTimeMs > 20000) {
      currentScreen = SCR_MAIN;
      uiRedraw = true;
    }
...
static void tftCentredText(const char *s, int x0, int x1, int y, uint16_t col,
                           uint8_t sz = 1, uint16_t bg = 0x0000) {
...
void drawStatusBar(int scr) {
  ...
  if (getLocalTime(&ti)) {
    char tb[9];
    snprintf(tb, 9, "%02d:%02d %s",
             ti.tm_hour < 12 ? ti.tm_hour : (ti.tm_hour - 12), ti.tm_min,
             ti.tm_hour < 12 ? "AM" : "PM");
    tft.setTextColor(C_WHITE, barBg);
    tft.setTextSize(1);
    tft.setCursor(3, 4);
    tft.print(tb);
  }
...
void drawFooter(const char *txt, uint16_t col = C_DKGRAY) {
  ...
  tft.setTextSize(1);
  int tw = strlen(txt) * 6;
  tft.setCursor(max(0, (SW - tw) / 2), 117);
  tft.print(txt);
}

// Professional rounded button
void drawBtn(int x, int y, int w, int h, const char *txt, uint16_t bgCol,
             uint16_t txtCol, bool active = false) {
  ...
  tft.setTextSize(1);
  int tw = strlen(txt) * 6;
  tft.setCursor(x + (w - tw) / 2, y + (h - 7) / 2);
  tft.print(txt);
}
...
void drawMotorIconPro(int cx, int cy, int r, bool running) {
  ...
  for (int a = 0; a < 360; a += 60) {
    ...
    int x2 = cx + (int)((r - 5) * cosf(rd)),
        y2 = cy + (int)((r - 5) * sinf(rd));
...
void drawShieldIcon(int x, int y, bool ok) {
  ...
  if (ok) {
    tft.setTextColor(col, ok ? 0x0280 : 0x5000);
    tft.setCursor(x + 4, y + 3);
    tft.print(F("v"));
  } else {
    tft.setTextColor(col, 0x5000);
    tft.setCursor(x + 4, y + 3);
    tft.print(F("!"));
  }
}
...
void drawWaterGaugePro(int cx, int cy, int r, float pct, bool running) {
  ...
  if (fillH > 2) {
    for (int wx = cx - r + 2; wx < cx + r - 2; wx++) {
      float wv = sinf(wx * 0.28f + r_waveOffset) * 1.5f +
                 sinf(wx * 0.45f - r_waveOffset * 0.6f) * 1.0f;
  ...
  char ps[7];
  snprintf(ps, 7, "%.0f%%", (float)((int)pct));
  int tw = strlen(ps) * 12;
  tft.setTextSize(2);
...
void drawRingGaugePro(int cx, int cy, int r, float pct, uint16_t col) {
  ...
  for (int a = 135; a <= 405; a += 3) {
    ...
    tft.drawPixel(cx + (int)((r - 1) * cosf(rd)),
                  cy + (int)((r - 1) * sinf(rd)), C_DKGRAY);
  }
  // Filled arc
  ...
  for (int a = 135; a <= 135 + deg; a += 2) {
    ...
    tft.drawPixel(cx + (int)((r - 1) * cosf(rd)),
                  cy + (int)((r - 1) * sinf(rd)), col);
    tft.drawPixel(cx + (int)((r - 2) * cosf(rd)),
                  cy + (int)((r - 2) * sinf(rd)), col);
  }
  char ps[5];
  snprintf(ps, 5, "%.0f%%", pct);
  tft.setTextColor(col, C_BG);
  tft.setTextSize(2);
  int tw = strlen(ps) * 12;
  tft.setCursor(cx - tw / 2, cy - 7);
...
static void uiThickArc(int cx, int cy, int r, int thickness, int startDeg,
                       int endDeg, uint16_t col) {
...
static void drawRefWifiIcon(int x, int y, bool connected) {
  ...
  uint16_t col = !connected
                     ? C_RED
                     : (bars >= 3 ? C_GREEN : (bars == 2 ? C_YELLOW : C_RED));
  tft.fillCircle(x, y + 12, 2, col);
  if (bars >= 1)
    uiThickArc(x, y + 10, 4, 1, 220, 320, col);
  if (bars >= 2)
    uiThickArc(x, y + 10, 7, 1, 220, 320, col);
  if (bars >= 3)
    uiThickArc(x, y + 10, 10, 1, 220, 320, col);
}
...
static void drawRefMotorIcon(int x, int y, bool running) {
  ...
  if (running)
    tft.fillRoundRect(x + 3, y + 2, 11, 7, 2, 0x0240);
...
static void drawRefWaterTank(int cx, int cy, int r, float pct) {
  ...
  uint16_t waterCol = (percent >= cfg.highThr)  ? C_GREEN
                      : (percent >= cfg.lowThr) ? C_CYAN
                      : (percent >= 10)         ? C_YELLOW
                                                : C_RED;
  ...
  for (int x = cx - r + 4; x <= cx + r - 4; x++) {
    ...
    if (bottom >= start)
      tft.drawFastVLine(x, start, bottom - start + 1, waterCol);
  }
...
void drawScreen0_Gauge(bool fullRedraw) {
  ...
  const char *modeText = opMode == MODE_AUTO     ? "AUTO"
                         : opMode == MODE_MANUAL ? "MAN"
                                                 : "MNT";
  uint16_t modeCol = opMode == MODE_AUTO     ? C_GREEN
                     : opMode == MODE_MANUAL ? C_YELLOW
                                             : C_ORANGE;
  tft.setTextSize(1);
  tft.setTextColor(C_CYAN, C_BG);
  tft.setCursor(84, 36);
  ...
  if (opMode == MODE_AUTO)
    drawAutoIcon(136, 54, modeCol);

  tft.setTextColor(C_CYAN, C_BG);
  tft.setCursor(84, 73);
  tft.print(F("MOTOR"));
  drawRefMotorIcon(88, 84, pumpRunning);
  tftCentredText(pumpRunning ? "ON" : "OFF", 116, 154, 85,
                 pumpRunning ? C_GREEN : C_RED, 1, C_BG);
  ...
  tft.print(F("TODAY USED"));
  tft.setTextColor(C_WHITE, C_BG);
  tft.setTextSize(1);
  char ul[12];
  snprintf(ul, 12, "%.0f L", dailyUsage);
  ...
  // Voltage (top-right, yellow)  e.g. "229V / AC"
  {
    ...
    char vb[8];
    snprintf(vb, 8, "%.0fV", smoothVoltage);
    tft.setTextColor(C_YELLOW, C_BG);
    tft.setTextSize(1);
    // right-align voltage value
    int vw = strlen(vb) * 6;
    tft.setCursor(SW - vw - 2, 19);
    tft.print(vb);
    tft.setTextColor(C_DKGREEN, C_BG);
    tft.setCursor(SW - 14, 27);
    tft.print(F("AC"));
  }

  // MODE badge
  {
    const char *mStr = opMode == MODE_AUTO     ? "AUTO"
                       : opMode == MODE_MANUAL ? "MANUAL"
                                               : "MAINT";
    uint16_t mCol = opMode == MODE_AUTO     ? C_GREEN
                    : opMode == MODE_MANUAL ? C_YELLOW
                                            : C_ORANGE;
    tft.fillRect(rx, 33, SW - rx - 1, 32, C_BG);
    tft.setTextColor(mCol, C_BG);
    tft.setTextSize(1);
    tft.setCursor(rx, 33);
    tft.print(F("MODE"));
    // Badge box
  ...
  // MOTOR section
  {
    ...
    tft.setCursor(rx + 24, 79);
    tft.print(pumpRunning ? F("ON") : F("OFF"));
    if (pumpRunning && fillEtaMin > 0) {
      char eb[10];
      snprintf(eb, 10, "~%.0fm", fillEtaMin);
      tft.setTextColor(C_CYAN, C_BG);
      tft.setCursor(rx + 24, 89);
      tft.print(eb);
    }
  }
...
void drawScreen0_Robot(bool fullRedraw) {
    ...
    if (night) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        tft.drawLine(143 + (int)(cosf(a) * 13), 28 + (int)(sinf(a) * 13),
                     143 + (int)(cosf(a) * 16), 28 + (int)(sinf(a) * 16),
                     SUN_RIM);
      }
    }
  }
  ...
  // Stars or birds
  if (night) {
    int sX[] = {10, 24, 39, 55, 72, 88,  106, 123, 137, 151,
                16, 34, 50, 67, 84, 101, 118, 132, 146, 58};
    int sY[] = {23, 34, 21, 39, 28, 44, 25, 38, 50, 43,
                56, 61, 52, 63, 57, 66, 59, 69, 61, 31};
    for (int i = 0; i < 20; i++) {
      int blink = (r_frame + i * 5) % 32;
      if (blink < 22) {
        uint16_t c = (i % 2 == 0) ? STAR_COL : C_YELLOW;
        tft.drawPixel(sX[i], sY[i], c);
        if (blink > 17) {
          tft.drawPixel(sX[i] - 1, sY[i], c);
          tft.drawPixel(sX[i] + 1, sY[i], c);
          tft.drawPixel(sX[i], sY[i] - 1, c);
          tft.drawPixel(sX[i], sY[i] + 1, c);
        }
      }
    }
    ...
    if (bX < 160) {
      ...
      if ((r_frame / 8) % 2 == 0) {
        tft.drawLine(bX - 4, bY, bX - 1, bY - 2, 0x0000);
        tft.drawLine(bX - 1, bY - 2, bX + 2, bY, 0x0000);
      } else {
        tft.drawLine(bX - 4, bY - 1, bX - 1, bY - 2, 0x0000);
        tft.drawLine(bX - 1, bY - 2, bX + 2, bY - 1, 0x0000);
      }
    }
  }
  ...
  // TANK (left, x=5)
  {
    ...
    if (fH > 2) {
      for (int wx2 = tX + 2; wx2 < tX + tW - 2; wx2++) {
        float wv = sinf((wx2 * 0.25f) + r_waveOffset) * 2 +
                   sinf((wx2 * 0.4f) - r_waveOffset * 0.6f) * 1.5f;
        int wy2 = wTop + (int)wv;
        if (wy2 >= tY && wy2 < tY + tH) {
          tft.drawPixel(wx2, wy2, WATER_TOP);
          if (wy2 + 1 < tY + tH)
            tft.drawPixel(wx2, wy2 + 1, WATER_MID);
        }
      }
    }
    tft.setTextSize(2);
    tft.setTextColor(C_WHITE, WATER_MID);
  ...
  // PUMP (center, x=68)
  {
    ...
    tft.fillTriangle(pX - 25, pBY - 23, pX - 25, pBY - 20, pX - 28, pBY - 21,
                     PUMP_HANDLE);
    tft.drawTriangle(pX - 25, pBY - 23, pX - 25, pBY - 20, pX - 28, pBY - 21,
                     ROBOT_DARK);
  ...
  if (r_pumpingDown && r_pumpHandleY > 12 && r_frame % 5 == 0) {
    for (int i = 0; i < 8; i++) {
      if (r_drops[i].life <= 0) {
        r_drops[i] = {68 - 28.0f, (float)(72 - 21),
                      float(random(15, 25)) / 10.0f, random(20, 35)};
        break;
      }
    }
  }
  for (int i = 0; i < 8; i++) {
    if (r_drops[i].life > 0) {
      ...
      r_drops[i].y += r_drops[i].vy;
      r_drops[i].x -= 0.5f;
      r_drops[i].life--;
      if (r_drops[i].y > 74 && r_drops[i].life > 5)
        r_drops[i].life = 5;
    }
  }
  ...
  // ROBOT (right, x=120)
  {
    ...
    int shake = 0;
    if (r_frame % 4 < 2)
      shake = (r_frame % 2) ? 1 : -1; // subtle pump shake
    rX += shake;
    int hY = rY - 14;
    // Body
    ...
    tft.fillCircle(rX, hY - 6, 2,
                   (r_frame % 20 < 10) ? SPARK_COL : PUMP_ACCENT);
    // Arms to pump handle
  ...
  // Status line at bottom
  {
    ...
    if (fillEtaMin > 0) {
      char eb[22];
      snprintf(eb, 22, "FILLING  ETA: %.0f min", fillEtaMin);
      tft.setTextColor(C_GREEN, C_BG);
      tft.setTextSize(1);
      int tw = strlen(eb) * 6;
      tft.setCursor((SW - tw) / 2, 120);
      tft.print(eb);
    } else {
      tft.setTextColor(C_GREEN, C_BG);
      tft.setTextSize(1);
      tft.setCursor(35, 120);
      tft.print(F("MOTOR RUNNING"));
    }
  }
}
...
void drawScreen1_Motor() {
  ...
  tft.setCursor(99, 64);
  tft.print(protOk ? F("ACTIVE") : F("FAULT!"));

  // ETA if pumping
  if (pumpRunning && fillEtaMin > 0) {
    char eb[18];
    snprintf(eb, 18, "ETA %.0f min", fillEtaMin);
    tft.setTextColor(C_DKGRAY, C_BG);
    tft.setCursor(82, 80);
    tft.print(eb);
  }
  ...
  // ON button — green
  drawBtn(2, 98, 48, 20, "  ON", mOn ? C_GREEN : 0x0280, mOn ? C_BG : C_LGRAY,
          mOn);
  // OFF button — red
  drawBtn(56, 98, 48, 20, " OFF", mOff ? C_RED : 0x5000,
          mOff ? C_WHITE : C_LGRAY, mOff);
  // AUTO button — blue
  drawBtn(110, 98, 48, 20, "AUTO", mAuto ? C_DKBLUE : C_DKGRAY,
          mAuto ? C_CYAN : C_LGRAY, mAuto);
}
...
static const Alert *uiLatestAlert() {
  for (int i = 0; i < ALERT_HISTORY; i++) {
    int idx =
        ((alertHead - 1 - i) % ALERT_HISTORY + ALERT_HISTORY) % ALERT_HISTORY;
    if (alertHistory[idx].active)
      return &alertHistory[idx];
  }
  return nullptr;
}

void drawAlertPopup() {
  uint16_t accent = alertPopupLevel == AL_CRITICAL
                        ? C_RED
                        : (alertPopupLevel == AL_WARNING ? C_ORANGE : C_CYAN);
  uint16_t bg = 0x0841;
  tft.fillRoundRect(8, 24, 144, 80, 6, bg);
  tft.drawRoundRect(8, 24, 144, 80, 6, accent);
  tft.drawRoundRect(10, 26, 140, 76, 5, C_DKGRAY);
  tft.fillTriangle(26, 41, 15, 63, 37, 63, accent);
  tft.fillRoundRect(24, 49, 4, 8, 1, bg);
  tft.fillCircle(26, 60, 1, bg);
  ...
  tft.setCursor(46, 49);
  tft.print(alertPopupMsg);

  char line1[22] = "";
  char line2[22] = "";
  strncpy(line1, alertPopupDetail, 21);
  line1[21] = '\0';
  if (strlen(alertPopupDetail) > 21) {
    strncpy(line2, alertPopupDetail + 21, 21);
    line2[21] = '\0';
  }
  ...
  tft.fillRoundRect(42, 90, 76, 10, 3, C_DKGRAY);
  tft.setTextColor(C_WHITE, C_DKGRAY);
  tft.setCursor(47, 92);
  tft.print(F("BTN DISMISS"));
}
...
void drawScreen2_Alerts() {
  ...
  if (!hasAlert) {
    tft.setCursor(46, 43);
    tft.print(F("NO ACTIVE ALERT"));
    tft.setTextColor(C_GRAY, C_BG);
    tft.setCursor(46, 57);
    tft.print(F("System running"));
    tft.setCursor(46, 68);
    tft.print(F("normally"));
    ...
    tft.print(la->detail);
    char ts[10];
    snprintf(ts, 10, "%02d:%02d", la->hour, la->minute);
    tft.setCursor(46, 68);
    tft.print(ts);
  } else {
    char v2[22];
    ...
    tft.setCursor(46, 43);
    tft.print(F("HIGH VOLTAGE"));
    tft.setTextColor(C_GRAY, C_BG);
    tft.setCursor(46, 57);
    tft.print(v2);
    tft.setCursor(46, 68);
    tft.print(F("Check supply"));
  }
...
void drawScreen3_Usage() {
  ...
  tft.setTextSize(1);
  tft.setTextColor(C_WHITE, C_BG);
  tft.setCursor(px + 5, py + 5);
  tft.print(F("TODAY"));
  tft.setTextColor(C_CYAN, C_BG);
  tft.setTextSize(2);
  char ub[10];
  snprintf(ub, 10, "%.0f", dailyUsage);
  tft.setCursor(px + 5, py + 14);
  tft.print(ub);
  tft.setTextSize(1);
  tft.setCursor(px + 40, py + 22);
  tft.print(F("L"));

  float sumDay = 0.0f;
  for (int i = 0; i < 24; i++)
    sumDay += hourlyUsage[i];
  float avgDay = sumDay / 24.0f;
  tft.setTextColor(C_WHITE, C_BG);
  tft.setCursor(px + 5, py + 39);
  tft.print(F("AVG"));
  tft.setTextColor(C_CYAN, C_BG);
  tft.setTextSize(2);
  char ab[10];
  snprintf(ab, 10, "%.0f", avgDay);
  tft.setCursor(px + 5, py + 47);
  tft.print(ab);
  tft.setTextSize(1);
  tft.setCursor(px + 28, py + 55);
  tft.print(F("L"));

  int peakHr = 0;
  float peakV = 0.0f;
  for (int i = 0; i < 24; i++) {
    if (hourlyUsage[i] > peakV) {
      peakV = hourlyUsage[i];
      peakHr = i;
    }
  }
  tft.setTextColor(C_WHITE, C_BG);
  tft.setCursor(px + 5, py + 67);
  tft.print(F("PEAK"));
  uiClockIcon(px + 10, py + 84, C_CYAN);
  tft.setTextColor(C_CYAN, C_BG);
  char pb[10];
  snprintf(pb, 10, "%02d-%02d", peakHr, (peakHr + 3) % 24);
  tft.setCursor(px + 18, py + 80);
  tft.print(pb);
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (bh < 2)
      bh = 2;
    int by = cy + ch - 2 - bh;
    tft.fillRoundRect(bx, by, barW, bh, 1,
                      (i >= 6 && i <= 9) ? C_CYAN : C_MDBLUE);
    bx += (barW + gap);
  }

  tft.setTextColor(C_DKGRAY, C_BG);
  tft.setCursor(cx, cy + ch + 3);
  tft.print(F("0"));
  tft.setCursor(cx + 31, cy + ch + 3);
  tft.print(F("12"));
  tft.setCursor(cx + 63, cy + ch + 3);
  tft.print(F("24"));
...
void drawScreen4_System() {
  ...
  drawSystemCardRow(8, uiLightningRowIcon, "VOLTAGE", voltB, C_YELLOW,
                    vOk ? C_WHITE : C_RED);
  drawSystemCardRow(37, uiWifiRowIcon, "WIFI STATUS", wOk ? "CONN" : "OFF",
                    wOk ? C_GREEN : C_RED, wOk ? C_GREEN : C_RED);
  drawSystemCardRow(66, uiSensorIcon, "SENSOR", sOk ? "NORMAL" : "FAULT",
                    sOk ? C_CYAN : C_RED, sOk ? C_GREEN : C_RED);
  drawSystemCardRow(95, uiInfoIcon, "FIRMWARE", FW_VERSION, C_CYAN, C_WHITE);
  return;
  ...
  SysRow rows[] = {
      ...
      {"FIRMWARE", true, FW_VERSION, FW_VERSION},
  ...
  for (int i = 0; i < N; i++) {
    int y = 20 + i * 20;
    if (i > 0)
      tft.drawFastHLine(2, y - 1, SW - 4, C_DKGRAY);
    ...
    case 0: // Soundwave bars
      ...
      break;
    case 1:
      drawMotorIconPro(12, y + 8, 8, !pumpTimedOut);
      break;
    case 2:
      drawWifiIconPro(12, y + 3, wOk);
      break;
    case 3:
      ...
      break;
    case 4:
      drawLightningIcon(4, y + 1, vOk ? C_YELLOW : C_RED);
      break;
    }
...
void drawScreen5_Smart() {
  ...
  char nextB[12];
  if (!inSchedule())
    snprintf(nextB, 12, "%02d:00 AM", MOTOR_BLOCK_END);
  else
    snprintf(nextB, 12, "IN WINDOW");
  const char *predS = predictedLevel < cfg.lowThr
                          ? "LOW"
                          : (predictedLevel < 60 ? "MED" : "HIGH");
  bool lActive = lpmSamples > 3;
  char limB[8];
  snprintf(limB, 8, "%.0fV", cfg.safeVHigh);

  uiClockIcon(15, 19, C_WHITE);
  tft.setTextColor(C_WHITE, C_BG);
  tft.setTextSize(1);
  tft.setCursor(26, 11);
  tft.print(F("NEXT FILL"));
  ...
  tft.setTextColor(C_WHITE, C_BG);
  tft.setCursor(26, 65);
  tft.print(F("LEARNING"));
  tft.setTextColor(C_MAGENTA, C_BG);
  tft.setCursor(26, 75);
  tft.print(lActive ? F("ACTIVE") : F("CAL"));
  tft.drawFastHLine(10, 87, 64, C_DKGRAY);

  drawLightningIcon(9, 95, C_YELLOW);
  tft.setTextColor(C_WHITE, C_BG);
  tft.setCursor(26, 92);
  tft.print(F("AC LIMIT"));
  tft.setTextColor(C_MAGENTA, C_BG);
  ...
  char pB[10];
  snprintf(pB, 10, "%.0f%%", predictedLevel);
...
void drawScreen6_Settings() {
  ...
  drawSettingsRow(12, "MODE", "auto/manual", uiGearIcon, settingsIndex == 0,
                  modeB);
  drawSettingsRow(34, "LOW", "auto start", uiBellIcon, settingsIndex == 1,
                  lowB);
  drawSettingsRow(56, "HIGH", "auto stop", uiClockIcon, settingsIndex == 2,
                  highB);
  drawSettingsRow(78, "RESET", "today used", uiResetIcon, settingsIndex == 3,
                  resetB);
  drawSettingsRow(100, "HEIGHT", "tank depth", uiTankSmallIcon,
                  settingsIndex == 4, tankB);
}
// ============================================================
//  DISPLAY TASK
// ============================================================
...
void DisplayTask(void *pv) {
  for (;;) {
    ...
    if (TAKE_MUTEX(xTftMutex, 50)) {
      ...
      if (alertPopupActive && millis() >= alertPopupUntil) {
        alertPopupActive = false;
        uiRedraw = true;
      }

      bool popupNow = alertPopupActive && millis() < alertPopupUntil;
      bool popupChanged = (popupNow != prevPopup);
      bool pumpChanged = (prevPump != pumpRunning);
      bool isMain = (currentScreen == SCR_MAIN);
      bool needFull = uiRedraw || (millis() - lastFullRedraw > 30000) ||
                      (isMain && pumpChanged) || popupChanged;
      bool valChanged =
          (fabsf(smoothLevel - prevLevel) > 0.5f ||
           fabsf(smoothVoltage - prevVolt) > 2.0f ||
           fabsf(dailyUsage - prevUsage) > 0.5f || prevPump != pumpRunning ||
           prevMqtt != mqtt.connected());

      bool anim = isMain && pumpRunning; // [V8-7] animation only when pumping

      if (needFull || valChanged || anim || popupNow) {
        if (needFull)
          lastFullRedraw = millis();
        switch (currentScreen) {
        case SCR_MAIN:
          if (pumpRunning)
            drawScreen0_Robot(needFull);
          else
            drawScreen0_Gauge(needFull);
          break;
        case SCR_ALERTS:
          drawScreen2_Alerts();
          break;
        ...
        case SCR_SMART:
          ...
          break;
        case SCR_SETTINGS:
          drawScreen6_Settings();
          break;
        }
        if (popupNow)
          drawAlertPopup();
        prevLevel = smoothLevel;
        prevVolt = smoothVoltage;
        prevUsage = dailyUsage;
        prevPump = pumpRunning;
        prevMqtt = mqtt.connected();
        prevPopup = popupNow;
        uiRedraw = false;
    ...
    vTaskDelay(
        pdMS_TO_TICKS((currentScreen == SCR_MAIN && pumpRunning) ? 65 : 200));
  }
}
...
void mqttPublishDebug() {
  if (!mqtt.connected() || !debugEnabled)
    return;

  char payload[360];
  String ip =
      WiFi.isConnected() ? WiFi.localIP().toString() : String("0.0.0.0");

  snprintf(payload, sizeof(payload),
           "{\"fw\":\"%s\",\"build\":\"%s "
           "%s\",\"chip\":\"%s\",\"cores\":%u,\"cpu_mhz\":%u,\"sdk\":\"%s\","
           "\"reset\":\"%s\",\"flash\":%u}",
           FW_VERSION, __DATE__, __TIME__, ESP.getChipModel(),
           ESP.getChipCores(), ESP.getCpuFreqMHz(), ESP.getSdkVersion(),
           resetReasonText(esp_reset_reason()), ESP.getFlashChipSize());
  mqtt.publish(T_DBG_INFO, payload, true);

  snprintf(payload, sizeof(payload),
           "{\"uptime_s\":%lu,\"heap\":%u,\"min_heap\":%u,\"max_alloc\":%u,"
           "\"wifi\":%s,\"rssi\":%d,\"ip\":\"%s\",\"mqtt\":%s,\"state\":\"%s\","
           "\"mode\":\"%s\",\"pump\":%s,\"sleep_req\":%s}",
           millis() / 1000, ESP.getFreeHeap(), ESP.getMinFreeHeap(),
           ESP.getMaxAllocHeap(), WiFi.isConnected() ? "true" : "false",
           WiFi.isConnected() ? WiFi.RSSI() : 0, ip.c_str(),
  ...
  snprintf(payload, sizeof(payload),
           "{\"level\":%.1f,\"raw_level\":%.1f,\"distance_cm\":%.1f,"
           "\"voltage\":%.1f,\"sensor_ok\":%s,\"volt_ok\":%s,\"leak_score\":%d,"
           "\"eta_min\":%.1f,\"slope\":%.2f}",
           smoothLevel, waterLevel, lastDistanceCm, smoothVoltage,
           (usN > 0 || maLevel.count > 0) ? "true" : "false",
           isVoltSafe(smoothVoltage) ? "true" : "false", lastLeakScore,
           fillEtaMin, currentSlope);
  mqtt.publish(T_DBG_SENS, payload);

  snprintf(
      payload, sizeof(payload),
      "{\"ac_v\":%.1f,\"meter\":\"ZMPT "
      "only\",\"motor_current\":\"not_measured\",\"motor_power\":\"not_"
      "measured\",\"esp_5v\":\"not_measured\",\"box_temp\":\"not_measured\"}",
      smoothVoltage);
  mqtt.publish(T_DBG_POWER, payload);

  unsigned long now = millis();
  snprintf(payload, sizeof(payload),
           "{\"sensor_lag_ms\":%lu,\"control_lag_ms\":%lu,\"button_lag_ms\":%"
           "lu,\"display_lag_ms\":%lu,\"mqtt_lag_ms\":%lu,\"mqtt_connects\":%"
           "lu,\"mqtt_fails\":%lu,\"alerts\":%d}",
           now - hbSensor, now - hbControl, now - hbButton, now - hbDisplay,
           now - hbMqtt, (unsigned long)mqttConnectCount,
           (unsigned long)mqttFailCount, activeAlerts);
  mqtt.publish(T_DBG_TASKS, payload);

  snprintf(payload, sizeof(payload),
           "{\"pump_start\":\"%s\",\"pump_stop\":\"%s\",\"very_low\":%s,\"volt_"
           "warn\":%s,\"leak\":%s,\"pump_timeout\":%s,\"dry_run_checked\":%s}",
           pumpStartReason, pumpStopReason, veryLow ? "true" : "false",
           voltWarn ? "true" : "false", leakConfirmed ? "true" : "false",
           pumpTimedOut ? "true" : "false", dryRunChk ? "true" : "false");
  mqtt.publish(T_DBG_EVENT, payload);
}

void mqttPublishConsolidated() {
  if (!mqtt.connected())
    return;

  char payload[1024]; // Large enough for consolidated JSON
  String ip =
      WiFi.isConnected() ? WiFi.localIP().toString() : String("0.0.0.0");

  // Consolidate key fields for the web app's live diagnostics
  snprintf(payload, sizeof(payload),
           "{\"fw\":\"%s\",\"build\":\"%s "
           "%s\",\"chip\":\"%s\",\"cpu_mhz\":%u,\"cores\":%u,"
           "\"uptime_sec\":%lu,\"heap\":%u,\"min_heap\":%u,\"rssi\":%d,\"ip\":"
           "\"%s\","
           "\"mqtt\":%s,\"state\":\"%s\",\"mode\":\"%s\",\"pump\":%s,"
           "\"level\":%.1f,\"distance_cm\":%.1f,\"voltage\":%.1f,"
           "\"leak_score\":%d,\"eta_min\":%.1f,\"slope\":%.2f,"
           "\"mqtt_reconnects\":%lu,\"mqtt_fails\":%lu,"
           "\"pump_start\":\"%s\",\"pump_stop\":\"%s\","
           "\"last_alert\":\"%s\"}",
           FW_VERSION, __DATE__, __TIME__, ESP.getChipModel(),
           ESP.getCpuFreqMHz(), ESP.getChipCores(), millis() / 1000,
           ...
           : opMode == MODE_MANUAL ? "MANUAL"
                                   : "MAINT",
           pumpRunning ? "true" : "false", smoothLevel, lastDistanceCm,
           smoothVoltage, lastLeakScore, fillEtaMin, currentSlope,
           (unsigned long)mqttConnectCount, (unsigned long)mqttFailCount,
           pumpStartReason, pumpStopReason,
           activeAlerts > 0 ? alertHistory[(alertHead - 1) % ALERT_HISTORY].msg
                            : "none");

  mqtt.publish(T_ADMIN_DBG, payload);
  MQTT_LOG("Consolidated snapshot published");
}
...
bool verifyHmac(const char *cmd, const char *tok) {
  ...
  mbedtls_md_hmac_starts(&ctx, (const uint8_t *)g_hmacSecret,
                         strlen(g_hmacSecret));
  ...
  char expected[65];
  for (int i = 0; i < 32; i++)
    sprintf(expected + i * 2, "%02x", result[i]);
  expected[64] = 0;
  return strncmp(expected, tok, 64) == 0;
}

void mqttCallback(char *topic, byte *payload, unsigned int length) {
  char msg[80] = {0};
  memcpy(msg, payload, min((unsigned int)79, length));
  char *pipe = strchr(msg, '|');
  if (!pipe) {
    MQTT_LOG("No HMAC");
    return;
  }
  *pipe = '\0';
  const char *cmd = msg;
  const char *tok = pipe + 1;
  if (!verifyHmac(cmd, tok)) {
    MQTT_LOG("HMAC fail");
    return;
  }
  if (!strcmp(topic, T_CMD_PMP)) {
    if (!strcmp(cmd, "ON") && opMode == MODE_MANUAL)
      startPump("MQTT command");
    else if (!strcmp(cmd, "OFF") && pumpRunning)
      requestPumpStop("MQTT command");
...
void mqttReconnect() {
  ...
  bool ok = mqtt.connect(MQTT_CLIENT_ID, cfg.mqttUser, cfg.mqttPass, T_STATUS,
                         1, true, "offline");
  if (ok) {
    mqttConnectCount++;
    mqtt.subscribe(T_CMD_PMP);
    mqtt.subscribe(T_CMD_MOD);
    mqtt.subscribe(T_CMD_RST);
    mqtt.subscribe(T_CMD_DBG);
    mqtt.publish(T_STATUS, "online", true);
    MQTT_LOG("Connected");
  } else {
    mqttFailCount++;
    MQTT_LOG("Failed rc=%d", mqtt.state());
  }
}
...
void mqttPublish() {
  if (!mqtt.connected())
    return;
  char buf[32];
  dtostrf(smoothLevel, 4, 1, buf);
  mqtt.publish(T_LEVEL, buf);
  dtostrf(smoothVoltage, 5, 1, buf);
  mqtt.publish(T_VOLT, buf);
  dtostrf(totalLitres, 6, 1, buf);
  mqtt.publish(T_USAGE, buf);
  dtostrf(fillEtaMin, 5, 1, buf);
  mqtt.publish(T_ETA, buf);
  mqtt.publish(T_PUMP, pumpRunning ? "ON" : "OFF", true);
  mqtt.publish(T_MODE, opMode == MODE_AUTO     ? "AUTO"
                       : opMode == MODE_MANUAL ? "MANUAL"
                                               : "MAINTENANCE");
  mqtt.publish(T_FSM_STATE, sysStateStr[sysState]);
  snprintf(buf, 32, "{\"runs\":%lu,\"hrs\":%.1f}", health.cycles,
           health.runSecs / 3600.0f);
  ...
  for (;;) {
    ...
    if (WiFi.isConnected() && mqttEnabled) {
      ...
      if (!mqtt.connected())
        mqttReconnect();
      mqtt.loop();
      if (millis() - mqttLastPub > MQTT_PUB_MS) {
        mqttLastPub = millis();
        mqttPublish();
      }
      if (debugEnabled && millis() - debugLastPub > DEBUG_PUB_MS) {
        debugLastPub = millis();
        mqttPublishDebug();
      }
    }
...
void setup() {
  ...
  delay(300);
  Serial.println(F("\n============================================"));
  Serial.println(F(" SMART WATER LEVEL INDICATOR  v9.2"));
  ...
  pinMode(PERIPH_5V_EN, OUTPUT);
  digitalWrite(PERIPH_5V_EN, HIGH);
  delay(500);
  ...
  if (wc == ESP_SLEEP_WAKEUP_EXT0)
    INFO("Woke: BTN1 pressed");
  else if (wc == ESP_SLEEP_WAKEUP_TIMER)
    INFO("Woke: 06:00 timer");
  else
    INFO("Normal boot");
  ...
  tft.setCursor(30, 22);
  tft.print(F("SMART WATER LEVEL"));
  tft.setCursor(46, 34);
  tft.print(F("v9.2 Safety"));
  tft.drawFastHLine(20, 44, 120, C_DKGRAY);
  tft.setTextColor(C_WHITE, C_DKGRAY);
  tft.setCursor(22, 50);
  tft.print(F("Initialising system..."));
  ...
  btn1.setButtonConfig(&btnCfg);
  btn2.setButtonConfig(&btnCfg);
  btn1.init(BTN1_PIN, LOW);
  btn2.init(BTN2_PIN, LOW);
  ...
  if (wm.autoConnect("WaterTank-Setup")) {
    INFO("WiFi OK: %s  IP:%s", WiFi.SSID().c_str(),
         WiFi.localIP().toString().c_str());
    strncpy(cfg.broker, pBroker.getValue(), 79);
    cfg.broker[79] = '\0';
    cfg.mqttPort = atoi(pPort.getValue());
    strncpy(cfg.mqttUser, pUser.getValue(), 31);
    cfg.mqttUser[31] = '\0';
    strncpy(cfg.mqttPass, pPass.getValue(), 39);
    cfg.mqttPass[39] = '\0';
    saveSettings();
    if (strlen(g_brokerCA) > 64)
      secClient.setCACert(g_brokerCA);
    else
  ...
  // Initial voltage
  {
    float s = 0;
    for (int i = 0; i < 80; i++) {
      s += (analogRead(VOLT_PIN) / 4095.0f) * 3.3f;
    ...
    maVolt.push(currentVoltage);
    smoothVoltage = currentVoltage;
  }
  ...
  INFO("All tasks started. System v9.2 running.");
}
